...
interface IUniswapV2Router{
    ...
    function swapTokensForExactTokens(uint amount, uint amountInMax, address[] calldata path, address to, uint deadline) external returns(uint[] memory amounts);
}

contract UniswapV2Examples{
    ...
    function swapMultipleHopExactAmountIn(uint amountIn, uint amountOutMin) external returns(uint amountOut){
    ...
    function swapSingleHopExactAmountOut(uint amountOutDesired, uint amountInMax) external returns(uint amountOut) {
        ...
        uint[] memory amounts = router.swapTokensForExactTokens(amountOutDesired, amountInMax, path, msg.sender, block.times);
